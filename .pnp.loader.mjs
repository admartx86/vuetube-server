...
async function copyPromise(destinationFs, destination, sourceFs, source, opts) {
    const normalizedDestination =
        destinationFs.pathUtils.normalize(destination);
...
async function copyImpl(
    ...
    if (
        opts.linkStrategy?.type !== `HardlinkFromIndex` ||
        !sourceStat.isFile()
    ) {
        if (
            updated ||
...
async function copyFileViaIndex(
    ...
    postlayout.push(async () => {
        if (!indexStat) {
            await destinationFs.lutimesPromise(
                indexPath,
                defaultTime,
                defaultTime
            );
...
class FakeFS {
    ...
    async checksumFilePromise(path, { algorithm = `sha512` } = {}) {
        ...
        try {
            ...
            let bytesRead = 0;
            while (
                (bytesRead = await this.readPromise(
                    fd,
                    chunk,
                    0,
                    CHUNK_SIZE
                ...
                await Promise.all(
                    entries.map(entry => {
                        return this.removePromise(
                            this.pathUtils.resolve(p, entry)
                        );
                    })
                );
            }
                ...
                if (opts.recursive && process.platform === `win32`) {
                    if (opts.withFileTypes) {
                        this.realFs.readdir(
                            ...
                            this.makeCallback(
                                results =>
                                    resolve(results.map(direntToPortable)),
                                reject
                            )
                        );
                    } else {
                        this.realFs.readdir(
                            ...
                            this.makeCallback(
                                results =>
                                    resolve(results.map(npath.toPortablePath)),
                                reject
                            )
                        );
                    }
            ...
            if (
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (
                        ...
                        const patternTrailer = StringPrototypeSlice(
                            key,
                            patternIndex + 1
                        );
                        if (
